...
- [x] **Image upload integrated into TipTap → Supabase Storage** ✅ DEPLOYED (PR #115) — bucket `blog-media`, sharp AVIF transcode, migrated `<img>` → `next/image`
- [x] **Email notifications (Resend → admin on pending_review, → author on approve/reject)** ✅ implemented (current PR) — 3 templates registry + dynamic admin emails lookup + try/catch decoupled wireup
- [ ] UI for admin to toggle `is_blogger` (currently manual SQL) — low priority hasta tener 2do blogger
