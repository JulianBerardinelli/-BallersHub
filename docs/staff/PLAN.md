...
- **P0.1 `roles[]` por etapa visibles + editables (M).** Lo más roto: se capturan en onboarding + se materializan, pero el SELECT del editor los descarta (`career-data.ts:92`), `CareerEditor` va con `showRole=false` sin campo `roles[]`, el render público no los recibe, y el diff admin no los pinta. Toca DB(✓)/dashboard/público/admin.
