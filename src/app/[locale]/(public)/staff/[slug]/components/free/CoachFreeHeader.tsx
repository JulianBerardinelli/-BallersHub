...
// CoachFreeHeader — fixed glassmorphic nav pill for the Free coach portfolio.
// Cloned from the player FreeHeader (same scroll-spy + share + back/powered
// chrome) with the section list swapped for the coach dossier sections.
...
// Sections that actually render in CoachFreeLayout. Locked Pro previews are
// not navigable on purpose. Labels come from the `portfolio.nav` namespace.
const COACH_SECTIONS: SectionItem[] = [
  { id: "bio", key: "philosophy" },
  { id: "career", key: "career" },
  { id: "licenses", key: "licenses" },
  { id: "contact", key: "contact" },
];

export default function CoachFreeHeader({
  fullName,
  avatarUrl,
}: {
  fullName: string;
  ...
  const handleShare = async () => {
    try {
      ...
      // Silently swallow user-cancelled share or clipboard denial.
    }
  };
  ...
  useEffect(() => {
    const onScroll = () => {
      // Don't highlight any section while the hero still dominates the viewport.
            ...
            return (
              <button
                ...
                className={`relative pointer-events-auto px-2.5 md:px-3 py-1.5 rounded-full text-[10px] md:text-xs font-semibold uppercase tracking-[0.18em] md:tracking-widest transition-colors whitespace-nowrap ${
                  isActive
                    ? "text-bh-black"
                    : "text-white/70 hover:text-bh-lime"
                }`}
              >
                {isActive && (
                  <motion.span
                    layoutId="coach-free-nav-active-pill"
                    className="absolute inset-0 rounded-full bg-bh-lime"
                    style={{
                      boxShadow:
                        "0 4px 22px color-mix(in srgb, #CCFF00 45%, transparent)",
                    }}
