...
export default function NationalTeamReviewList({
  items,
  canModerate,
}: {
  items: PendingStint[];
  // Solo los admin pueden aprobar/rechazar (las rutas usan ensureAdminActor,
  // admin-only). Los roles read-only (analyst) ven la cola sin acciones.
  canModerate: boolean;
}) {
          ...
          <div key={s.id} className="space-y-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
            ...
            {canModerate ? (
              ...
              <p className="text-xs text-white/40">
                Solo lectura — solo un administrador puede aprobar o rechazar.
              </p>
            )}
          </div>
        );
