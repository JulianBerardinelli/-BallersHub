            ...
            <li key={a.id} className="rounded-lg border border-neutral-800 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate font-medium">{a.full_name ?? "(sin nombre)"}</p>
                  <p className="text-xs text-neutral-500">
                    {new Date(a.created_at).toLocaleString("es-AR", { hour12: false })}
                  </p>
                  <div className="mt-1 flex items-center gap-2">
                    <Chip size="sm" variant="flat" color={
                      a.plan_requested === "free" ? "default" :
                        a.plan_requested === "pro" ? "primary" : "warning"
                    }>
                      {a.plan_requested}
                    </Chip>
                    {a.transfermarkt_url && (
                ...
                <form action={`/api/admin/applications/${a.id}/approve`} method="post">
                  <Button size="sm" color="primary" type="submit" aria-label="Aprobar solicitud">
                    Approve
                  </Button>
                </form>
              </div>
