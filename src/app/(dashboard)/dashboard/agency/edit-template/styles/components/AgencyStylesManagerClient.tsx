...
const TEMPLATE_OPTIONS = [
  {
    id: "classic",
    name: "Clásica",
    description: "Tarjeta institucional con logo, datos y roster en una sola página.",
  },
  {
    id: "pro",
    name: "Pro Agency (3D)",
    description: "Hero cinematográfico con parallax, marquee y módulos premium. Ideal para agencias destacadas.",
  },
];
...
const TYPOGRAPHY_BY_LAYOUT: Record<string, { id: string; label: string }[]> = {
  classic: [
  ...
  pro: [
    ...
    { id: "michroma", label: "Michroma (Tech)" },
  ],
};
...
export default function AgencyStylesManagerClient({ agency, initialTheme }: Props) {
  ...
  const [layout, setLayout] = useState(initialTheme?.layout ?? "classic");
  ...
  const [typography, setTypography] = useState(initialTheme?.typography ?? "inter");
  ...
  useEffect(() => {
    const validTypos = TYPOGRAPHY_BY_LAYOUT[layout] || TYPOGRAPHY_BY_LAYOUT["classic"];
    ...
    try {
      ...
      enqueue(
        profileNotification.updated({
          ...
          changedFields: ["Plantilla", "Paleta", "Tipografía", "Hero"],
        }),
      );
    } catch (e) {
      console.error(e);
  ...
  const fontsForLayout = TYPOGRAPHY_BY_LAYOUT[layout] || TYPOGRAPHY_BY_LAYOUT["classic"];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="flex items-start gap-3 rounded-bh-md border border-[rgba(245,158,11,0.25)] bg-[rgba(245,158,11,0.08)] p-4 text-bh-warning">
            ...
            <div>
              ...
              <p className="mb-2 mt-1 text-[12px] leading-[1.55] text-bh-warning/80">
                El layout Pro lo muestra como sello en la portada. Si no lo cargás, usamos un monograma con tus iniciales.
              </p>
              <Link
                href="/dashboard/agency"
                className="text-[12px] font-bold underline-offset-4 hover:underline"
              >
                Subir logo en la edición de la agencia &rarr;
              </Link>
            </div>
          </div>
        )}
        ...
        <SectionCard
          ...
          description="Elegí cómo se verá tu portfolio público en /agency/[slug]."
        >
              ...
              >
                ...
                <div className="relative flex aspect-video w-full items-center justify-center overflow-hidden rounded-bh-md border border-dashed border-white/[0.08] bg-gradient-to-br from-bh-surface-2 to-bh-surface-1">
                  {opt.id === "pro" ? (
                    ...
                    <span className="truncate font-bh-display text-3xl font-black tracking-widest text-bh-fg-1 opacity-20">
                      ROSTER
                    </span>
                  )}
                </div>
        ...
        <SectionCard
          ...
          description="Aplicá una paleta predefinida o personalizá los colores corporativos de la agencia."
        >
        ...
        <SectionCard
          title="Tipografía"
          description="Elegí el par tipográfico que mejor se ajuste al estilo de la plantilla."
        >
          <div className="grid gap-3 md:grid-cols-2">
            {fontsForLayout.map((font) => (
              ...
              >
                <p className="font-bh-heading text-base font-bold text-bh-fg-1">{font.label}</p>
                <p className="mt-1 text-[11px] text-bh-fg-3">
                  Familia recomendada para el layout {TEMPLATE_OPTIONS.find((l) => l.id === layout)?.name}
                </p>
              </div>
            ))}
          </div>
        ...
        {layout === "pro" && (
          <SectionCard
            ...
            description="Definí el texto cinematográfico que reemplaza al nombre por defecto en la portada."
          >
