// Modera una idea de juego (pre-moderación: pending → approved/rejected).
// Espejo de coach-methodology/[id]/moderate, sin cascada de archivos (la idea
// es self-contained: pizarra + texto). El render público exige status='approved'.
...
export async function POST(req: Request, ctx: { params: Params }) {
  ...
  if (!idea) return NextResponse.json({ error: "game idea not found" }, { status: 404 });
  ...
  revalidatePath("/admin/coach-game-ideas");
