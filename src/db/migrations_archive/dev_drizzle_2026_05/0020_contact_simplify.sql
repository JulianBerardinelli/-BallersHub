-- Simplify portfolio public contact: only WhatsApp + auto auth email.
-- Drop the per-channel show flags + the social URL columns + public_email.
-- Keep `whatsapp` and `show_contact_section`.
-- The view must be dropped before the columns (it references them).
...
ALTER TABLE "player_personal_details"
  ...
  DROP COLUMN IF EXISTS "show_instagram",
  DROP COLUMN IF EXISTS "show_linkedin";
