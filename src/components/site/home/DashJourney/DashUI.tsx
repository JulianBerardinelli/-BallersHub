...
export const ProfileScreen = ({ accent = "#CCFF00", h = 460, media = {} }: { accent?: string; h?: number; media?: MediaConfig }) => {
  const t = useTranslations("home.dashJourney.mock.profile");
  const stats: ReadonlyArray<[string, string]> = [
    ["23", t("stats.matches")],
    ["9", t("stats.goals")],
    ["1.94m", t("stats.height")],
  ];
  return (
        ...
        <div style={{ position: "absolute", left: 30, bottom: -38, display: "flex", alignItems: "flex-end", gap: 18 }}>
          ...
          <div style={{ paddingBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              ...
              <span style={{ fontFamily: _BARLOW, fontWeight: 700, fontSize: 12, letterSpacing: "0.05em", color: "#22C55E", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 7, padding: "3px 10px" }}>{t("verifiedChip")}</span>
            </div>
          </div>
        </div>
        <div style={{ position: "absolute", right: 26, top: 22, display: "flex", gap: 9 }}>
          <span style={{ fontFamily: _DM, fontWeight: 600, fontSize: 13, color: "#080808", background: accent, borderRadius: 9, padding: "8px 16px" }}>{t("contactCta")}</span>
        </div>
      </div>
      ...
      <div style={{ padding: "18px 30px", display: "flex", flexDirection: "column", gap: 11 }}>
        <div style={{ position: "relative", aspectRatio: "16/9", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
          ...
          <span style={{ position: "absolute", left: 12, bottom: 10, display: "flex", alignItems: "center", gap: 7, fontFamily: _DM, fontSize: 11, fontWeight: 600, color: "#fff", textShadow: "0 1px 4px rgba(0,0,0,0.6)" }}>
            <span style={{ width: 22, height: 22, borderRadius: "50%", background: accent, display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="9" height="9" viewBox="0 0 24 24" fill="#080808"><path d="M8 5v14l11-7z" /></svg></span>{t("reelLabel")}
          </span>
        </div>
        {[1, 2].map((i) => (
...
export const ProfilePhone = ({ accent = "#CCFF00", media = {} }: { accent?: string; media?: MediaConfig }) => {
  const t = useTranslations("home.dashJourney.mock.profilePhone");
  const phoneStats: ReadonlyArray<[string, string]> = [
    ["23", t("statAbbrev.matchesPlayed")],
    ["9", t("statAbbrev.goals")],
    ["4.8", t("statAbbrev.rating")],
  ];
  return (
    <div style={{ position: "absolute", inset: 0, background: "#0a0b0d", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ paddingTop: 38, textAlign: "center" }}>
        ...
        <div style={{ fontFamily: _DM, fontSize: 11, color: accent, marginTop: 4 }}>{t("role")}</div>
      </div>
      <div style={{ display: "flex", gap: 8, padding: "16px 16px 0" }}>
        {phoneStats.map(([n, l], i) => (
      ...
      <div style={{ margin: 14, padding: "12px 0", textAlign: "center", borderRadius: 12, background: accent, fontFamily: _DM, fontWeight: 700, fontSize: 13, color: "#080808" }}>{t("contactCta")}</div>
    </div>
  );
};
...
export const DashboardScreen = ({ accent = "#CCFF00", previewAccent, h = 520, media = {} }: { accent?: string; previewAccent?: string; h?: number; media?: MediaConfig }) => {
  const t = useTranslations("home.dashJourney.mock.dashboard");
  const pa = previewAccent || accent;
  const sidebarItems = [
    t("sidebar.items.profile"),
    t("sidebar.items.career"),
    t("sidebar.items.stats"),
    t("sidebar.items.media"),
    t("sidebar.items.visualIdentity"),
    t("sidebar.items.reviews"),
  ];
  return (
    <div style={{ height: h, display: "flex", background: "#0a0b0d", overflow: "hidden" }}>
      <div style={{ width: 188, borderRight: "1px solid rgba(255,255,255,0.06)", padding: "18px 14px", display: "flex", flexDirection: "column", gap: 6, background: "rgba(255,255,255,0.015)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 14, padding: "0 6px" }}>
          ...
          <span style={{ fontFamily: _DM, fontSize: 10, color: "rgba(255,255,255,0.4)" }}>{t("sidebar.label")}</span>
        </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ height: 50, display: "flex", alignItems: "center", gap: 12, padding: "0 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ fontFamily: _BARLOW, fontWeight: 800, fontSize: 21, color: "#fff", textTransform: "uppercase" }}>{t("header.title")}</div>
          <span style={{ fontFamily: _DM, fontSize: 11, color: "rgba(255,255,255,0.36)" }}>{t("header.subtitle")}</span>
        ...
        <div style={{ flex: 1, padding: 22, display: "grid", gridTemplateColumns: "1fr 0.82fr", gap: 18, overflow: "hidden" }}>
          ...
          <div style={{ borderRadius: 14, overflow: "hidden", border: `1px solid ${pa}3a`, background: "#070809", position: "relative", boxShadow: `0 0 50px -28px ${pa}` }}>
            <div style={{ height: 22, display: "flex", alignItems: "center", gap: 6, padding: "0 10px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <span style={{ fontFamily: _DM, fontSize: 9, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: pa }}>{t("previewLabel")}</span>
...
export const AgentDashScreen = ({ accent = "#00C2FF", h = 520 }: { accent?: string; h?: number }) => {
  const t = useTranslations("home.dashJourney.mock.agent");
  const sidebarItems = [
    t("sidebar.items.roster"),
    t("sidebar.items.tracking"),
    t("sidebar.items.dossiers"),
    t("sidebar.items.market"),
    t("sidebar.items.messages"),
  ];
  const kpis: ReadonlyArray<readonly [string, string, string]> = [
    ["€1.95M", t("kpis.portfolioValue"), accent],
    ["+14%", t("kpis.trend90d"), "#22C55E"],
    ["28", t("kpis.interestedClubs"), "#fff"],
  ];
  const tableHeaders = [t("table.player"), t("table.club"), t("table.value"), t("table.trend")];
  return (
    <div style={{ height: h, display: "flex", background: "#070809", overflow: "hidden" }}>
      <div style={{ width: 188, borderRight: "1px solid rgba(255,255,255,0.06)", padding: "18px 14px", display: "flex", flexDirection: "column", gap: 6, background: "rgba(255,255,255,0.012)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 14, padding: "0 6px" }}>
          <span style={{ fontFamily: _BARLOW, fontWeight: 900, fontSize: 20, color: "#fff" }}><span style={{ color: accent }}>&apos;</span>BH</span>
          <span style={{ fontFamily: _DM, fontSize: 10, color: accent }}>{t("sidebar.label")}</span>
        </div>
        {sidebarItems.map((it, i) => (
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ height: 50, display: "flex", alignItems: "center", gap: 12, padding: "0 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ fontFamily: _BARLOW, fontWeight: 800, fontSize: 21, color: "#fff", textTransform: "uppercase" }}>{t("header.title")}</div>
          <span style={{ fontFamily: _DM, fontSize: 11, color: accent }}>{t("header.subtitle")}</span>
        ...
        <div style={{ padding: "16px 22px", flex: 1 }}>
          <div style={{ display: "grid", gridTemplateColumns: "2.2fr 1.2fr 0.9fr 0.9fr", gap: 8, padding: "0 12px 9px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            {tableHeaders.map((hd) => <span key={hd} style={{ fontFamily: _DM, fontSize: 9.5, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.34)" }}>{hd}</span>)}
          </div>
...
export const PagePreviewPhone = ({ accent = "#00C2FF", media = {} }: { accent?: string; media?: MediaConfig }) => {
  const t = useTranslations("home.dashJourney.mock.page");
  const pageStats: ReadonlyArray<[string, string]> = [
    ["18", t("statAbbrev.goals")],
    ["680K", t("statAbbrev.euros")],
    ["22", t("statAbbrev.age")],
  ];
  return (
    <div style={{ position: "absolute", inset: 0, background: "#0a0b0d", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ paddingTop: 36, textAlign: "center" }}>
        ...
        <div style={{ display: "inline-flex", gap: 6, marginTop: 8 }}>
          <span style={{ fontFamily: _DM, fontSize: 9, fontWeight: 700, color: accent, border: `1px solid ${accent}55`, borderRadius: 6, padding: "3px 8px" }}>{t("officialRep")}</span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 8, justifyContent: "flex-end" }}>
        <div style={{ padding: "11px 0", textAlign: "center", borderRadius: 12, background: accent, fontFamily: _DM, fontWeight: 700, fontSize: 13, color: "#080808" }}>{t("requestDossier")}</div>
      </div>
    </div>
  );
};
