...
/* ── 1 · brand colours ── */
const BRAND_PRESETS = [
  { key: "Monocromo", grad: "linear-gradient(150deg,#2c2c2c,#070707)", colors: { bg: "#0A0A0B", primary: "#FFFFFF", secondary: "#3A3A3E", accent: "#EDEDED" } },
  { key: "Cyberpunk", grad: "linear-gradient(150deg,#E5277F,#7B2FF7)", colors: { bg: "#0D0212", primary: "#FF2D9B", secondary: "#7B2FF7", accent: "#FF6AD5" } },
  { key: "Azul Eléctrico", grad: "linear-gradient(150deg,#3F7BF6,#15307A)", colors: { bg: "#050A14", primary: "#2A6FDB", secondary: "#15307A", accent: "#4D9DFF" } },
  { key: "Oro Imperial", grad: "linear-gradient(150deg,#F59E0B,#7C2D12)", colors: { bg: "#0D0502", primary: "#FA8A00", secondary: "#7C2D12", accent: "#FFB347" } },
];
...
export const BrandColorsCard = ({ accent = "#CCFF00", onPick, style }: { accent?: string; onPick?: (hex: string) => void; style?: CSS }) => {
  ...
  return (
    <div className="dj-card" style={{ width: 430, padding: 22, ...style }}>
      <div style={{ fontFamily: BARLOW, fontWeight: 800, fontSize: 23, letterSpacing: "0.01em", textTransform: "uppercase", color: "#fff" }}>Colores de marca</div>
      <p style={{ fontFamily: DM, fontSize: 13, color: "rgba(255,255,255,0.46)", margin: "5px 0 18px", lineHeight: 1.45 }}>Elegí una paleta preestablecida o definí los colores manualmente.</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 20 }}>
        {BRAND_PRESETS.map((p, i) => (
          <button key={p.key} type="button" onClick={() => { setSel(i); onPick?.(p.colors.accent); }}
            ...
            <span style={{ fontFamily: DM, fontSize: 12.5, fontWeight: 600, color: i === sel ? "#fff" : "rgba(255,255,255,0.6)" }}>{p.key}</span>
            <span style={{ fontFamily: DM, fontSize: 8.5, fontWeight: 700, letterSpacing: "0.18em", color: i === sel ? accent : "rgba(255,255,255,0.28)" }}>PRESET</span>
          </button>
        ))}
      </div>
      <div style={{ display: "flex", gap: 16, marginBottom: 14 }}>
        <HexRow label="Background" hex={c.bg} />
        <HexRow label="Primario" hex={c.primary} />
      </div>
      <div style={{ display: "flex", gap: 16 }}>
        <HexRow label="Secundario" hex={c.secondary} />
        <HexRow label="Acento" hex={c.accent} />
      </div>
    </div>
  );
};
  ...
  <div className="dj-card" style={{ width: 560, padding: 22, ...style }}>
    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.08)", marginBottom: 14 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontFamily: DM, fontSize: 12.5, color: "rgba(255,255,255,0.4)" }}>Juveniles AFA · 2015–2017</div>
      </div>
    </div>
    <div style={{ padding: 18, borderRadius: 16, border: `1px solid ${accent}38`, background: `${accent}07`, boxShadow: `inset 0 0 40px -24px ${accent}` }}>
      <div style={{ display: "flex", gap: 13, marginBottom: 16 }}>
        <Field label="Club"><Select accent={accent} value="CA Independiente" crest={<span style={{ width: 22, height: 22, borderRadius: 5, background: "linear-gradient(#E11,#7a0a0a)", flexShrink: 0, border: "1px solid rgba(255,255,255,0.2)" }} />} /></Field>
        <Field label="División" w={132}><Select accent={accent} value="Liga Pro" /></Field>
        <Field label="Desde (año)" w={96}><Select accent={accent} value="2018" /></Field>
        <Field label="Hasta (año)" w={96}><Select accent={accent} value="2019" /></Field>
      </div>
      <div style={{ display: "flex", gap: 26, justifyContent: "flex-end", marginBottom: 18 }}>
        <Toggle accent={accent} label="Es mi equipo actual" />
        <Toggle accent={accent} label="Disputé otra categoría/liga" />
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: BARLOW, fontWeight: 700, fontSize: 13, letterSpacing: "0.05em", textTransform: "uppercase", color: "#22C55E", border: "1px solid rgba(34,197,94,0.4)", background: "rgba(34,197,94,0.08)", borderRadius: 9, padding: "8px 14px" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth="2.4"><path d="M20 6L9 17l-5-5" /></svg>Equipo verificado
        </span>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
          <GhostBtn>Cancelar</GhostBtn>
          <GhostBtn fill accent={accent}>Confirmar etapa</GhostBtn>
          <GhostBtn danger>Eliminar</GhostBtn>
        </div>
      </div>
    </div>
    <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
      <GhostBtn fill accent={accent}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6"><path d="M12 5v14M5 12h14" /></svg>Agregar etapa
      </GhostBtn>
    </div>
  </div>
);
...
export const StatsCard = ({ accent = "#CCFF00", secondary = "#00C2FF", style }: { accent?: string; secondary?: string; style?: CSS }) => (
  <div className="dj-card" style={{ width: 560, padding: 24, ...style }}>
    <div style={{ fontFamily: BARLOW, fontWeight: 800, fontSize: 23, letterSpacing: "0.01em", textTransform: "uppercase", color: "#fff" }}>Estadísticas por temporada</div>
    <p style={{ fontFamily: DM, fontSize: 12.5, color: "rgba(255,255,255,0.44)", margin: "5px 0 16px", lineHeight: 1.45 }}>Seguimiento agregado de tus números oficiales para compartir con clubes y representantes.</p>
    <div style={{ display: "grid", gridTemplateColumns: "2.4fr 1fr 0.6fr 0.6fr 0.6fr", gap: 8, padding: "0 6px 9px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
      {["Temporada", "Competencia", "PJ", "GOL", "ASI"].map((h) => (
        <span key={h} style={{ fontFamily: DM, fontSize: 9.5, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.34)", textAlign: h === "Temporada" || h === "Competencia" ? "left" : "center" }}>{h}</span>
      ))}
    </div>
    {STAT_ROWS.map((r, i) => (
      <div key={i} className="dj-statrow" style={{ display: "grid", gridTemplateColumns: "2.4fr 1fr 0.6fr 0.6fr 0.6fr", gap: 8, alignItems: "center", padding: "11px 6px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <span style={{ fontFamily: DM, fontSize: 13, color: "rgba(255,255,255,0.66)" }}>Liga</span>
    ...
    <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 13 }}>
      <div style={{ display: "flex", gap: 13 }}>
        <Field label="Etapa de trayectoria"><Select accent={secondary} value="CA Independiente · 2018–2019" /></Field>
        <Field label="Temporada" w={150}><TextInput accent={secondary} placeholder="Ej: 2023 / 2024" /></Field>
      </div>
      <div style={{ display: "flex", gap: 13 }}>
        <Field label="Equipo"><TextInput accent={secondary} placeholder="Club" /></Field>
        <Field label="PJ" w={70}><TextInput accent={secondary} value="18" align="center" /></Field>
        <Field label="Titular" w={70}><TextInput accent={secondary} value="16" align="center" /></Field>
        <Field label="Goles" w={70}><TextInput accent={secondary} value="5" align="center" /></Field>
        <Field label="Asist." w={70}><TextInput accent={secondary} value="3" align="center" /></Field>
      </div>
      <div style={{ display: "flex", justifyContent: "flex-start", marginTop: 2 }}>
        <button type="button" style={{ fontFamily: DM, fontWeight: 600, fontSize: 14.5, padding: "11px 22px", borderRadius: 10, border: `1px solid ${secondary}66`, background: `${secondary}12`, color: secondary, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8 }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6"><path d="M12 5v14M5 12h14" /></svg>Agregar temporada
        </button>
      </div>
    </div>
  </div>
